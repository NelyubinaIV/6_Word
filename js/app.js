...
function fitMaskedWords(){
  const title=document.querySelector('.task-console h1');
  if(!title) return;
  const availableWidth=title.clientWidth;

  title.querySelectorAll('.masked-word').forEach(word=>{
    word.style.removeProperty('--masked-word-scale');
    const naturalWidth=word.scrollWidth;
    if(naturalWidth<=availableWidth) return;
    const scale=Math.max(.52,Math.min(1,availableWidth/naturalWidth));
    word.style.setProperty('--masked-word-scale',`${scale}em`);
  });
}
...
function renderResults(){const r=state.results||{}, title=state.sessionType==='learning'?'МИССИЯ ЗАВЕРШЕНА':state.sessionType==='review'?'ПОВТОРЕНИЕ ЗАВЕРШЕНО':'ОТРАБОТКА ЗАВЕРШЕНА'; const isDifficult=state.sessionType==='difficult'; const items=isDifficult?[['Освоено',r.recoveredCount??0],['Остались сложными',r.stillDifficultCount??0]]:[state.sessionType==='review'?['Сразу верно',r.correctFirstTry??0]:['С первой попытки',r.correctFirstTry??0],['Сложные слова',r.difficultCount??0]]; const showEarned=state.sessionType==='learning'||isDifficult; const content=`<section class="result-card"><div class="result-star">★</div><span class="eyebrow">${title}</span><h1>Отличная работа,<br>${escapeHTML(displayName())}!</h1>${showEarned?`<div class="earned">⭐ +${r.starsEarned??0}</div>`:''}<div class="result-stats two">${items.map(([l,v])=>`<div><b>${v}</b><span>${l}</span></div>`).join('')}</div><button class="cta" id="home">Вернуться в кабинет <span>→</span></button></section>`;app.innerHTML=shell(content);bindShell();document.querySelector('#home').onclick=loadDashboard;}
...
window.addEventListener('resize',fitMaskedWords);
